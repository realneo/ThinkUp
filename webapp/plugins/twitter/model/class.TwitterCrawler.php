...
class TwitterCrawler {
    ...
    private function fetchAndAddTweetRepliedTo($tid) {
        ...
        if (isset($this->user)) {
            ...
            if ($cURL_status == 200) {
                $tweet = $this->api->parseJSONTweet($twitter_data);
    ...
    public function fetchRetweetsOfInstanceUser() {
        ...
        if (isset($this->user)) {
            ...
            list($cURL_status, $twitter_data) = $this->api->apiRequest($rtsofme, $args);
            if ($cURL_status == 200) {
                $tweets = $this->api->parseJSONTweets($twitter_data);
                foreach ($tweets as $tweet) {
                    try {
    ...
    public function fetchInstanceFavorites() {
        ...
        $continue = true;
        while ($continue) {
            list($tweets, $cURL_status, $twitter_data) = $this->getFavorites($last_fav_id);
            if ($cURL_status == 200) {
                if (sizeof($tweets) == 0) {
    ...
    private function getFavorites($since_id) {
        $favs_call = $this->api->cURL_source['favorites'];
        $args = array();
