...
class TwitterAPIAccessorOAuth {
    ...
    public function verifyCredentials() {
        $auth_url = $this->cURL_source['credentials'];
        $args = array('include_entities'=>'false', 'skip_status'=>'true');

        $twitter_data = $this->to->OAuthRequest($auth_url, 'GET', $args);
        $cURL_status = $this->to->lastStatusCode();

        if ($cURL_status == 200) {
            $user = $this->parseJSONUser($twitter_data);
            return $user;
        } else {
            throw new APIErrorException(self::translateErrorCode($cURL_status, true));
        }
    }
    /**
    ...
    protected function prepAPI() {
        ...
        $api_domain = 'https://api.twitter.com/1.1';
        $api_format = 'json';

        // Define method paths ... [id] is a placeholder
        //@TODO delete the methods that are not in use
        $api_method = array(
